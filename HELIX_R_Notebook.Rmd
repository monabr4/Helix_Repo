...
grouped_lasso <- cv.grpreg(x_train, y_train, group = adjusted_groups, family = "binomial", penalty = "grLasso", lambda.min.ratio = 1e-5)
...
grouped_ridge <- cv.grpreg(x_train, y_train, group = adjusted_groups, family = "binomial", penalty = "gel", lambda.min.ratio = 1e-5)
...
cv_glmnet_model <- cv.glmnet(x_train, y_train, family = "binomial", alpha = 0.5, lambda = 10^seq(-4, 4, length.out = 100))
...
In this analysis, three different regression models (Lasso, Ridge, and
Elastic Net) were applied to the Helix dataset, which excludes
metabolomics data.

The models were trained on 80% of the data and tested on the remaining
20%. For the Lasso model, the Area Under the Curve (AUC) was 0.592, with
an accuracy of 69.6%. The Ridge model yielded an AUC of 0.584 and the
same accuracy of 69.6%. The Elastic Net model resulted in an AUC of
0.589 and an accuracy of 68.5%. Overall, while the accuracies were
similar across models, the Lasso model performed slightly better in terms of AUC compared to Ridge and Elastic Net.
...
# Two models (covariates and covariates + diet)
...
fit_model3 <- grpreg(X_model3, y, group = group_structure_model3, penalty = "grLasso")
cv_fit_model3 <- cv.grpreg(X_model3, y, group = group_structure_model3, penalty = "grLasso", nfolds = 10)
best_lambda_model3 <- cv_fit_model3$lambda.min
best_fit_model3 <- grpreg(X_model3, y, group = group_structure_model3, penalty = "grLasso", lambda = best_lambda_model3)
...
fit_model4 <- grpreg(X_model4, y, group = group_structure_model4, penalty = "grLasso")
cv_fit_model4 <- cv.grpreg(X_model4, y, group = group_structure_model4, penalty = "grLasso", nfolds = 10)
best_lambda_model4 <- cv_fit_model4$lambda.min
best_fit_model4 <- grpreg(X_model4, y, group = group_structure_model4, penalty = "grLasso", lambda = best_lambda_model4)
...
predictors <- c(covariates_final, phthalates_final, diet_final, metabolites_final)
y <- as.factor(full_data_with_meta_imputed$hs_bmi_c_cat)

# make that matrix!
X <- full_data_with_meta_imputed[, predictors]

# Fit the Random Forest model 
set.seed(2024)  
rf_model <- randomForest(X, y, importance = TRUE, ntree = 500, maxnodes = 30, nodesize = 10)
...
predictions <- predict(rf_model, X, type = "prob")[,2]
